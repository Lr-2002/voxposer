...
from .utils_main import normalize_vector
...
class Controller:
    ...
    def _apply_mpc_control(self, control, grasp_quat=None, target_velocity=1):
        """
        ...
        ee_quat = self._calculate_ee_rot(pushing_dir)# if grasp_quat is None else grasp_quat
        print()
        print('******I am apply mpc control******')
        print()
        # if self.env.ee_quat != None:
        #     ee_quat = self.env.ee_quat
    ...
    def execute(self, movable_obs, waypoint):
        ...
        # optimize through dynamics model to obtain robot actions
            ...
            self._apply_mpc_control(best_control[0], grasp_quat=target_rotation)
    ...
    def random_shooting_MPC(self, start_obs, target):
        ...
        print('[Info] using MPC to shoot for obs ')
        obs_sequences = []
        controls_sequences = []
        costs = []
        info = dict()
