...
class PushingDynamicsModel:
    ...
    def forward(self, inputs, max_per_batch=1000):
    ...
    def _forward_batched(self, inputs):
        ...
        is_outward = (
            np.sum((obj_center - contact_position) * gripper_direction, axis=1) < 0
        )  # B
        ...
        output = (
            pcs + moving_dist[:, np.newaxis, :] * gripper_direction[:, np.newaxis, :]
        )  # B x N x 3
        return output
