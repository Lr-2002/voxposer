...
from .utils_main import (Observation, VoxelIndexingWrapper, bcolors, get_clock_time,
                    normalize_vector, pointat2quat, rotation_matrix_to_quaternion,
...
class LMP_interface():
  ...
  def detect(self, obj_name):
    ...
    if obj_name.lower() in EE_ALIAS:
      ...
      obs_dict = dict()
      obj_pc, normal = self._env.get_3d_obs_by_name(obj_name)
      ...
      obs_dict['name'] = obj_name
      obs_dict['position'] = self._world_to_voxel(np.mean(obj_pc , axis=0) -np.array([0, 0, 0.02]))   # in voxel frame 
      obs_dict['aabb'] = np.array([aabb_min, aabb_max])  # in voxel frame
      obs_dict['_position_world'] = np.mean(obj_pc, axis=0)-np.array([0, 0, 0.01])  # in world frame
      obs_dict['_point_cloud_world'] = obj_pc  # in world frame
      obs_dict['normal'] = normalize_vector(normal.mean(axis=0))#normal_vector #normalize_vector(obj_normal.mean(axis=0))
      # obs_dict['normal'] = normalize_vector(np.array([0,0,1]))#normal_vector #normalize_vector(obj_normal.mean(axis=0))
      # obs_dict['normal'] = np.array([0,1, 0, 0])#normal_vector #normalize_vector(obj_normal.mean(axis=0))
      print(f"\033[91m {obj_name} normal is {np.mean(normal, axis=0)}  \033[0m")
      print(f"\033[91m {obj_name} position is {np.mean(obj_pc, axis=0)}  \033[0m")

    object_obs = Observation(obs_dict)
    print(obj_name, obs_dict['position'], obs_dict['_position_world'])
    # input()
    return object_obs
  # new interface
  def estimate_grasp_pose(self, query_name):
    obs_dict = {}
    obj_pc, obj_normal = self._env.get_3d_obs_by_name(query_name)
    tran, rot = self._env.get_grasp_pose_graspnet(obj_pc, obj_normal, query_name)
    tran = tran + rot[:, 0] * 0.006
    tran_voxel = self._world_to_voxel(tran)
    quat = mat2quat(rot)
    obs_dict.update({'translation': tran_voxel, 'rotation': rot, 'translation_world': tran, 'quat': quat})
    object_obs = Observation(obs_dict)
    return object_obs

  def filter_trajectory(self, traj_world):
    # print(f'last traj waypoint is {traj_world[-1]}')
    rotation = quat2mat(traj_world[-1][1])
    # print(f'filter rotation is {rotation}')
    direction = rotation[:, 2].reshape(3)
    # print(f'direction is {direction}')
    quat = traj_world[-1][1]
    velocity = traj_world[-2][2]
    grip = 1
    norm_thresold = 0.07
    interpolate_point = traj_world[-1][0] - norm_thresold * direction
    half_interpolate_point = traj_world[-1][0] - norm_thresold * 0.5 * direction
    half_waypoint = (half_interpolate_point, quat, velocity, grip)
    inter_waypoint = (interpolate_point, quat, velocity, grip)
    for i in range(len(traj_world) - 2, -1, -1):
      waypoint = traj_world[i]
      if np.sqrt(np.sum((waypoint[0] - traj_world[-1][0]) ** 2)) < norm_thresold:
        traj_world.pop(i)
      else:
        break
    traj_world.insert(len(traj_world) - 1, inter_waypoint)
    traj_world.insert(len(traj_world) - 1, half_waypoint)
    return traj_world
  # new interface
  def add_end_waypoint(self, waypoints):
    # for i in range(len(waypoints)):
    #   waypoints[i][3] = 1
    for id, waypoint in enumerate(waypoints):
      waypoint = list(waypoint)
      waypoint[3] = 1
      waypoints[id] = tuple(waypoint)
    quat = waypoints[-1][1]
    ...
    flag = -1
    gripper = 1
    velocity = waypoints[-1][2]
    position = waypoints[-1][0]
    for iter in range(10):
      current_position = position - 0.1 * flag * (iter + 1) / 10 * direction
      if iter != 9:
        waypoints.append((current_position, quat, velocity, gripper))
      else:
        gripper = 0
        waypoints.append((current_position, quat, velocity, gripper))
    return waypoints
  ...
  def vec2quat(self, vec):
    ...
    angle = np.arccos(np.clip(np.dot(ref, vec), -1.0, 1.0))  # Clip for numerical stability
    
    # Handle edge cases
    if np.linalg.norm(axis) < 1e-6:  # No rotation needed
        if np.dot(ref, vec) > 0:  # Same direction as z-axis
            return [0, 0, 0, 1]
        else:  # Opposite direction to z-axis
            return [1, 0, 0, 0]  # 180-degree rotation about any x-y axis
  ...
  def _path2traj(self, path, rotation_map, velocity_map, gripper_map):
    ...
    for i in range(len(path)):
        ...
        voxel_xyz = np.round(voxel_xyz).astype(int)
            #
        rotation = np.array([0, 1, 0, 0 ]) #TODO up down rotation 
