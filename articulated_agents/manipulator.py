...
class Manipulator(ArticulatedAgentInterface):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ee link consraints
        self._snap_constaints = []

        # defaults for optional params
        if self.params.gripper_init_params is None:
            self.params.gripper_init_params = np.zeros(len(self.params.gripper_joints), dtype=np.float32)
        if self.params.arm_init_params is None:
            self.params.arm_init_params = np.zeros(len(self.params.arm_joints), dtype=np.float32)
    ...
    def arm_action(self, delta_pos, arm_id):
        # clip from -1 to 1
        delta_pos = np.clip(delta_pos, -1, 1)
        delta_pos *= 0.0125

        cur_pos = self.arm_motor_pos[arm_id]
        self._set_motor_pos(self.params.arm_joints[arm_id], cur_pos + delta_pos)
    ...
    def reconfigure(self) -> None:
        ...
        for link_id in self.sim_obj.get_link_ids():
            self.joint_pos_indices[link_id] = self.sim_obj.get_link_joint_pos_offset(link_id)
            ...
            for cam_prefix, sensor_names in self._cameras.items():
                for sensor_name in sensor_names:
                    ...
                    if cam_info.attached_link_id == -1:
                        ...
                        link_trans = self.sim_obj.get_link_scene_node(cam_info.attached_link_id).transformation
                    ...
                    sens_obj.node.transformation = orthonormalize_rotation_shear(cam_transform)
    ...
    def arm_joint_limits(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        lower_lims = np.array([lower_joints_limits[i] for i in arm_pos_indices], dtype=np.float32)
        upper_lims = np.array([upper_joint_limits[i] for i in arm_pos_indices], dtype=np.float32)
    ...
    def ee_link_id(self, ee_index: int = 0) -> int:
        ...
        if ee_index >= len(self.params.ee_links):
            raise ValueError("The current manipulator does not have enough end effectors")
    ...
    def ee_transform(self, ee_index: int = 0) -> mn.Matrix4:
        ...
        ef_link_transform = self.sim_obj.get_link_scene_node(self.params.ee_links[ee_index]).transformation
        ef_link_transform.translation = ef_link_transform.transform_point(self.ee_local_offset(ee_index))
    ...
    def set_gripper_target_state(self, gripper_state: float) -> None:
        ...
        for i, jidx in enumerate(self.params.gripper_joints):
            delta = self.params.gripper_closed_state[i] - self.params.gripper_open_state[i]
    ...
    def is_gripper_open(self) -> bool:
        ...
        return (
            np.amax(np.abs(self.gripper_joint_pos - np.array(self.params.gripper_open_state)))
    ...
    def is_gripper_closed(self) -> bool:
        ...
        return (
            np.amax(np.abs(self.gripper_joint_pos - np.array(self.params.gripper_closed_state)))
            < self.params.gripper_state_eps
        )
    ...
    def arm_joint_pos(self):
        ...
        arm_pos_indices = (joint_pos_indices[x] for x in arm_joints)
        return np.array([sim_obj_joint_pos[i] for i in arm_pos_indices], dtype=np.float32)
    ...
    def _interpolate_arm_control(self, targs, idxs, seconds, ctrl_freq, get_observations=False):
        ...
        for i in range(T):
            ...
            for j, jidx in enumerate(idxs):
                ...
                joint_positions[self.joint_pos_indices[jidx]] = delta[j] * (i + 1) + curs[j]
            self.sim_obj.joint_positions = joint_positions
    ...
    def snap_by_collision(self, grasp_obj_ids):
        """
        Snap the contacted object. 
        grasp_obj_ids: list of ids of snapable objects
        """
        ...
        for scene_obj_id in grasp_obj_ids:
            for c in robot_contacts:
                if coll_name_matches(c, scene_obj_id):
                    match_coll = c
                    break
            if match_coll is not None:
                self.grasp_by_id(scene_obj_id)
                return True

    def snap_by_id(self, attempt_snap_entity, force=False):
        """
        Snap an object through the object id.
        attempt_snap_entity: Object id. It should be an DYNAMIC object.
        force: If true, the object will be transformed to the end effector,
        no matter how far it is from the robot.
        """
        rom = self._sim.get_rigid_object_manager()
        ro = rom.get_object_by_id(attempt_snap_entity)

        # here we need the link T, not the EE T for the constraint frame
        ee_link_T = self.sim_obj.get_link_scene_node(
            self.params.ee_links[0]
        ).absolute_transformation()

        if force:
            ro.transformation = self.ee_transform()

        c = RigidConstraintSettings()
        c.object_id_a = self.sim_obj.object_id
        c.link_id_a = self.params.ee_links[0]
        c.object_id_b = attempt_snap_entity
        c.pivot_a = ee_link_T.inverted().transform_point(ro.translation)
        c.pivot_b = mn.Vector3.zero_init()
        c.frame_a = mn.Matrix3.identity_init()

        c.max_impulse = 10000.0
        c.constraint_type = RigidConstraintType.Fixed

        # we set the link frame to object rotation in link space (objR -> world -> link)
        link_id = self.params.ee_links[0]
        link_node = self.sim_obj.get_link_scene_node(link_id)
        ...
        object_frame_world_space = (
            ...
            .get_object_by_id(attempt_snap_entity)
        ...
        self._snap_constaints.append(self._sim.create_rigid_constraint(c))

    def desnap(self):
        """
        Remove all constraints of the articulated agent.
        """
        for constraint_id in self._snap_constaints:
