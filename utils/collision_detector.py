...
class ModelFreeCollisionDetector():
    ...
    def solve_grasp_dict(self, grasp_dict):
        height_list, width_list, depth_list = [], [], []
        translation_list = []
        rotation_list = []
        for key in grasp_dict.keys():
            grasp = grasp_dict[key]
            height, width, depth = 0, 0, 0
            if 'height' not in grasp.keys():
                height, width, depth = 0.02, 0.01, 0.06
            else:
                height, width, depth = grasp['height'], grasp['width'], grasp['depth']
            height_list.append(height)
            width_list.append(width)
            depth_list.append(depth)
            translation_list.append(grasp['position'])
            rotation_list.append(grasp['rotation'])
            
        height, width, depth = np.array(height_list), np.array(width_list), np.array(depth_list)
        translation, rotation = np.array(translation_list), np.array(rotation_list)
        
        # grasp_group.translation = translation_list
        return height, width, depth, translation, rotation
    ...
    def detect(self, grasp_group, approach_dist=0.03, collision_thresh=0.05, return_empty_grasp=False, empty_thresh=0.01, return_ious=False):
        ...
        height, width, depth, translation, rotation = self.solve_grasp_dict(grasp_group)
        T = translation #grasp_group.translations
        R = rotation #grasp_group.rotation_matrices
        heights = height[:, np.newaxis] #grasp_group.heights[:,np.newaxis]
        depths = depth[:, np.newaxis] #grasp_group.depths[:,np.newaxis]
        widths = width[:, np.newaxis] #grasp_group.widths[:,np.newaxis]
        ...
        # print(f'global_iou is {global_iou}')
        print(f'global iou maximum value is {np.max(global_iou)}')
        print(f'global iou minimum value is {np.min(global_iou)}')
        # get collison mask
        # print('all global iou are')
        print(global_iou)
        return global_iou
